...
import ..SDDGraphics: Colors, color, colortype, _ColorType, _coloringfunction, _width, _height,
    _xmax, _xmin, _ymax, _ymin, pointtocanvas, _drawingkind, _axes

#using Colors

colortype(Colors.RGBA{Float64})
#global _ColorType = Colors.RGBA{Float64} # Luxor uses colors with alpha
_bgcolor = _ColorType(1,1,1,0) # White completely transparent
_fgcolor = _ColorType(0,0,0,1) # Black completely opaque


color(c::_ColorType) = Luxor.setcolor(c)
color(x::Real, y::Real) = Luxor.setcolor(_coloringfunction(x,y))
color(z::Number) = Luxor.setcolor(_coloringfunction(z))
color() = RGBA(
    Luxor.get_current_redvalue,
    Luxor.get_current_greenvalue,
    Luxor.get_current_bluevalue,
    Luxor.get_current_alphavalue
)
...
Luxor.arc2r(Luxor.Point(x1,y1),Luxor.Point(x2,y2),Luxor.Point(x3,y3))
drawarc(z1::Number, z2::Number, z3::Number) =
Luxor.arc2r(Luxor.Point(real(z1),imag(z1)),Luxor.Point(real(z2),imag(z2)),Luxor.Point(real(z3),imag(z3)))
drawarc(a::AbstractArc) =
Luxor.arc2r(Luxor.Point(centerR2(a)...),Luxor.Point(initialpointR2(a)...),Luxor.Point(finalpointR2(a)...))
